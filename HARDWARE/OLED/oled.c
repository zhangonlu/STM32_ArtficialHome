...
需要增加: const unsigned char HZK_Str[] = "王老师";
const unsigned char Hzk1[][32]= {
	{0x00,0x02,0x82,0x82,0x82,0x82,0x82,0xFE,0x82,0x82,0x82,0x82,0x82,0x02,0x00,0x00,0x40,0x40,0x40,0x40,0x40,0x40,0x40,0x7F,0x40,0x40,0x40,0x40,0x40,0x40,0x40,0x00},"王",0
	{0x20,0x20,0x24,0x24,0x24,0x24,0xBF,0x64,0x24,0x34,0x28,0x24,0x22,0x20,0x20,0x00,0x10,0x08,0x04,0x02,0x3F,0x45,0x44,0x44,0x42,0x42,0x42,0x41,0x78,0x00,0x00,0x00},"老",0
	{0x00,0xFC,0x00,0x00,0xFF,0x00,0x02,0xE2,0x22,0x22,0xFE,0x22,0x22,0xE2,0x02,0x00,0x00,0x87,0x40,0x30,0x0F,0x00,0x00,0x1F,0x00,0x00,0xFF,0x08,0x10,0x0F,0x00,0x00},"师",1
}
****************************************************************/
#include "string.h"
...
void OLED_ShowChineseString(u8 x,u8 y,char *data,u8 size1,u8 mode)
{
    u8 _x = x;
    u8 _y = y;
    int num = 0;
    while(*data)
    {
        ...
        {
            OLED_ShowChar(_x,_y,*data,size1,mode);
            _x+= size1/2;
        }
        /*----- 往后偏移 --------*/
        data++;
...
void OLED_MyClear(void)//清屏函数不刷新
{
    u8 i,n;
    for(i=0; i<8; i++)
    {
        for(n=0; n<128; n++)
        {
            OLED_GRAM[n][i]=0;//清除数据
        }
    }
}

void OLED_My_clear_choose(u32 num)//清除选项
{
    u8 i,n;
    for(i=0; i<2; i++)
    {
        for(n=0; n<30; n++)
        {
            OLED_GRAM[n][i+num*2]=0;//清除数据
        }
    }
}

void OLED_My_clear_cinese(u32 num)//清除选项中文
{
    u8 i,n;
    for(i=0; i<2; i++)
    {
